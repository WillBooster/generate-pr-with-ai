...
${responseFence}`;
  ...
  if (!options.dryRun) {
    ...
    if (issueInfo.base_branch) {
      prCreateArgs.push('--base', issueInfo.base_branch);
    }
    await runCommand('gh', prCreateArgs);
  } else {
    console.info(ansis.yellow(`Would create PR with title: ${prTitle}`));
    if (issueInfo.base_branch) {
      console.info(ansis.yellow(`Would create PR against base branch: ${issueInfo.base_branch}`));
    }
    console.info(
      ansis.yellow(
